  ...
  <header style="background: #fff; border-bottom-left-radius: 32px; border-bottom-right-radius: 32px; box-shadow: 0 2px 12px #0001; overflow: hidden;">
    <div style="position: relative;">
      ...
      <div style="position: absolute; top: 16px; right: 16px; display: flex; gap: 12px; align-items: center;">
        ...
        <button v-if="isAdmin" @click="handleLogout" style="margin-left: 10px; padding: 6px 16px; border-radius: 8px; border: none; background: #F39C12; color: #fff; font-weight: bold; cursor: pointer;">{{ t('logout') }}</button>
      </div>
    </div>
    <div style="padding: 20px 24px 0 24px;">
      <h1 style="margin: 0 0 8px 0; font-size: 2rem; font-weight: bold;">{{ props.restaurant?.name || t('header.demo') }}</h1>
